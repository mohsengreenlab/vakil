...
function renderCasesTable(casesData = null) {
    ...
    dataToRender.forEach(caseItem => {
        ...
        viewButton.style.marginLeft = '24px'; // 24px spacing for better visibility
        ...
        if (isClosed) {
            ...
            closedBadge.className = 'text-gray-500 text-sm px-2 py-1 bg-gray-200 rounded font-medium';
            closedBadge.textContent = 'بسته شده';
            actionsCell.appendChild(closedBadge);
        } else {
            // If case is open, show close button (added second for RTL layout - will appear on left)
